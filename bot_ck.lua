...
-- Prevents your bot from doing multiple actions
-- 防止代理一次执行多个操作。
InAction = false     
...
function decideNextAction()
  ...
  for target, state in pairs(LatestGameState.Players) do
    if target ~= ao.id and inRange(player.x, player.y, state.x, state.y, 1) then
      ...
      if state.health < player.health  then
        bestTarget = state
      end
    end
  end
  
  -- Attack if the player's energy is greater than 5 and there is a targeted enemy attached
  -- 如果玩家能量大于5，且附件有目标敌人则攻击
  if player.energy > 5 and targetInRange then
    ...
    -- Randomized movement in 8 directions
    -- 随机朝8个方向移动
    local directionMap = {"Up", "Down", "Left", "Right", "UpRight", "UpLeft", "DownRight", "DownLeft"}
    local randomIndex = math.random(#directionMap)
    ao.send({Target = Game, Action = "PlayerMove", Player = ao.id, Direction = directionMap[randomIndex]})
  end
  -- Reset the "InAction" flag is flase
  InAction = false 
end
